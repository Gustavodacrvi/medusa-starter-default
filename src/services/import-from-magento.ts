...
import { ProductCollectionService, ProductService, ShippingProfileService } from "@medusajs/medusa"
...
interface BackofficeCategory {
	category: string;
	description: string;
	keywords: any[];
	parent_category: string | null;
	seo_descriptions: any[];
	seo_headlines: any[];
	seo_url: string;
	uid: string;
}

interface BackofficeProduct {
	name: string;
	...
	bundle_products: any[];
	configurable_products: any[];
}

interface BackofficeFile {
	caption: string | null;
	fid: string;
	is_main: boolean;
	sequence: number;
...
class ImportFromMagento extends BaseService {
	...
	constructor({ productCollectionService, productService, shippingProfileService }) {
		super();
	...
	async saveCookie(url: string, email: string, password: string) {
		const res = (await axios.post(`${url}/login`, { email, password }))
		this.cookie = res.headers['set-cookie'][0]
		this.axiosConfig = { headers: { cookie: this.cookie } }
	}
	
	async importCategories(url: string) {
		const newUidMap: {
			[oldUid: string]: string;
		} = {}
		
		const { data: { results } } = await axios.get<{ results: BackofficeCategory[] }>(`${url}/api/categories`, this.axiosConfig)
		for (const category of results) {
			if (category.parent_category) continue
		...
		for (const child of results) {
			if (!child.parent_category) continue
	...
	async initiateImport({ url, email, password }: {
		url: string;
		email: string;
		password: string;
	}) {
		...
		for (const cat of categories ){
			...
			const { data: { results: products } } = await axios.get<{ results: BackofficeProduct[] }>(`${url}/api/products?page_size=50&page=1categories[]=${cat.uid}`)
...
export default ImportFromMagento;